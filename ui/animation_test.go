...
func TestBuildPointerFrames_EastStraight(t *testing.T) {
	...
	assertFrameCells(t, frames[0].Cells, []OverlayCell{
		{X: 4, Y: 1, R: '>'},
		{X: 3, Y: 1, R: '─'},
		{X: 2, Y: 1, R: '─'},
	})
	assertFrameCells(t, frames[1].Cells, []OverlayCell{
		{X: 5, Y: 1, R: '>'},
		{X: 4, Y: 1, R: '─'},
		{X: 3, Y: 1, R: '─'},
	})
	assertFrameCells(t, frames[2].Cells, []OverlayCell{
		{X: 6, Y: 1, R: '>'},
		{X: 5, Y: 1, R: '─'},
		{X: 4, Y: 1, R: '─'},
	})
	assertFrameCells(t, frames[3].Cells, []OverlayCell{
		{X: 7, Y: 1, R: '>'},
		{X: 6, Y: 1, R: '─'},
		{X: 5, Y: 1, R: '─'},
	})
...
func assertFrameCells(t *testing.T, got, want []OverlayCell) {
	t.Helper()
	if len(got) != len(want) {
